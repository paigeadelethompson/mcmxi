...
@plugin.command('social')
@plugin.command('social')
@plugin.example(f'.social')
def social_list(bot, trigger):
    """List all available Social APIs."""
    bot.say(f'Available Social APIs (6):')
    for i, api in enumerate(APIS[:10], 1):  # Show first 10
        bot.say(f"{i}. {api['name']} - {api['description'][:50]}")
    if len(APIS) > 10:
        bot.say(f'... and {len(APIS) - 10} more. Use .social_info <name> for details')


@plugin.command('social_info')
@plugin.example(f'.social_info <name>')
def social_info(bot, trigger):
    """Get information about a specific Social API."""
    if not trigger.group(2):
        bot.notice(trigger.nick, f'Usage: .social_info <api_name>')
        return
...
@plugin.command('social_search')
@plugin.example(f'.social_search <query>')
def social_search(bot, trigger):
    """Search Social APIs by name or description."""
    if not trigger.group(2):
        bot.notice(trigger.nick, f'Usage: .social_search <query>')
        return
