...
def shorten_url_anycast(url: str) -> Optional[str]:
    ...
    logger.warning('Anycast failed for all services')
    return None
...
def shorten(bot, trigger):
    ...
    if service not in valid_services:
        bot.notice(trigger.nick, 'Invalid service. Available: cleanuri, spoo, isgd, vgd, tinyurl, clcis, anycast, roundrobin')
        return
...
def setup(bot):
    ...
    register_apis('url_shorteners', APIS)
