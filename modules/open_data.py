...
@plugin.example(f'.open_data')
def open_data_list(bot, trigger):
    """List all available Open Data APIs."""
    bot.say(f'Available Open Data APIs (19):')
...
@plugin.example(f'.open_data_info <name>')
def open_data_info(bot, trigger):
    """Get information about a specific Open Data API."""
    if not trigger.group(2):
        bot.notice(trigger.nick, f'Usage: .open_data_info <api_name>')
        return
...
@plugin.example(f'.open_data_search <query>')
def open_data_search(bot, trigger):
    """Search Open Data APIs by name or description."""
    if not trigger.group(2):
        bot.notice(trigger.nick, f'Usage: .open_data_search <query>')
        return
...
def university_universitieslist(bot, trigger):
    ...
    url = f'https://raw.githubusercontent.com/Hipo/university-domains-list/master/world_universities_and_domains.json'
